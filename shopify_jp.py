...
價格策略：
  1. JSON-LD priceCurrency == JPY → 直接用
  2. 否則（SGD等外幣）→ data-selected-variant price cents ÷ 100
庫存/Variants：JSON-LD hasVariant（含 InStock/OutOfStock）
圖片：JSON-LD image（完整 URL）
"""
import json
import re
    ...
    async def _scrape_shopify_jp(self, url: str) -> ProductInfo:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                # ── 1. data-selected-variant（price = JPY cents，一定可靠）
                sv_match = re.search(
                    r'data-selected-variant[^>]*>(\{[^<]+\})',
                    html
                )
                price_from_sv = 0
                if sv_match:
                    try:
                        ...
                        price_from_sv = (sv.get("price") or 0) // 100
                    except Exception:
                        pass

                # ── 2. JSON-LD ProductGroup
                all_ld = re.findall(
                ...
                currency = first_offers.get("priceCurrency", "")
                ld_price = int(float(first_offers.get("price", "0")))

                if currency == "JPY":
                    product.price_jpy = ld_price
                    print(f"[Shopify DEBUG] JSON-LD JPY price={ld_price} → ¥{product.price_jpy}")
                else:
                    product.price_jpy = price_from_sv
                    print(f"[Shopify DEBUG] JSON-LD {currency} price={ld_price}，改用 data-selected-variant cents÷100 → ¥{product.price_jpy}")
