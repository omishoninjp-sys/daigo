...
### 4. 🔴 想到一個理由，就當場掃過同類的每一條

**寫下規則 A 的理由時，要問「這個理由還適用於哪裡」，然後當場全部改掉 ——
不是留給下次維護。** 理由寫得越清楚，越容易以為「寫下來就等於處理了」。

2026-09-07 的實例：`detect_restricted_category` 六條規則與二手豁免機制
**是同一個 commit 寫的**，註解也把理由寫得很明白 ——
...
if _ONEPIECE_CARD.search(haystack):      # ← 同樣的理由成立，但沒套
    return ("hard", _MSG_ONEPIECE)
if _CHUSEN.search(haystack):             # ← 同上
    return ("hard", _MSG_CHUSEN)
```

「二手平台上的是現貨，買得到」對寶可夢卡牌、航海王卡牌、抽選販售、
予約／受注生產**每一條都成立**，卻只套在一番賞一條上。

**46 小時後**，客人在 Mercari 的寶可夢卡牌商品頁上收到這則錯誤訊息：

> 這類商品目前不開放直接下單。**請改貼 Mercari、駿河屋、PayPay フリマ
> 上的現貨連結**，那些我們照常代購。

叫他去 Mercari，然後擋掉 Mercari。`jp.mercari.com/item/m26961089339`（¥8,999），
六分鐘內重試五次，第五次把 `/zh-TW` 拿掉再試 —— 他以為是網址格式的問題。

**為什麼這種漏會活下來**：硬擋不建商品、不成單，所以在訂單與商品兩個樣本裡
**完全隱形**（12 個月訂單回測、5,298 件商品全掃，都是 0 筆）。
唯一看得到的是 scrape_log 裡「爬成功但沒建頁、而且同一個網址重試多次」。
跟低估售價、誤擋客人一樣，**都是沒有回饋迴路的那一類**。

做法：新增或修改一條規則時，把同一個檔案裡**判斷形態相同**的規則列出來
（這裡是「標題關鍵字類」vs「純網域類」），逐條問理由適不適用，
在 commit 訊息或註解裡寫出「掃過哪些、哪些不適用、為什麼」。
**「只有這條要改」也是結論，但要寫出來，不能是預設。**
