...
# 🔴 措辭刻意不講死（2026-09-08 當天就被打臉一次）：
#    上一版寫「**已經不存在**」「再貼一次同一條連結**還是會一樣**」。
#    當天下午回頭驗那條被重試 13 次的 zozo 連結
#    （/sp/shop/ellnoloset/goods/110151716/）時，它**已經復活**了 ——
#    爬得出「Ellno Loset ワイドパンツ ¥8,800」。
#    暫時下架後又上架是真實情況，那兩句話就變成錯的承諾。
#    現在說「目前找不到」「短時間內多半還是一樣」——
#    要客人別一直重試的效果一樣，但沒有承諾這件商品永遠不會回來。
MSG_PRODUCT_GONE = (
    "這個商品頁在日本站上目前找不到（回應 404），通常是商品下架、售完，"
    "或賣家換了網址 —— 短時間內再貼同一條連結多半還是一樣，先不用一直重試。"
...
#
#   🔴 但那個 0 **不代表中籤品轉賣不存在** —— 是這條 regex 打不到二手的寫法。
#      `_CHUSEN` 的每一個樣式（抽選販売／抽選受付／抽選予約／【抽選】…）
#      都是**官方商品頁的用字**；二手賣家寫的是「当選品」，而 `当選` 不在
#      這條 regex 裡。實例（2026-09-09 線上實測，唯讀 /api/scrape）：
#          jp.mercari.com/item/m85369919814  ¥16,666
#          「当選品 モンスターハンター 一番くじ ハンティングトロフィー ダブルチャンス」
#      那件真實存在、正在賣，但它是靠 `_ICHIBAN`（一番くじ）命中、
#      再由二手豁免放行的 —— 行為正確，卻**不是 `_CHUSEN` 抓到的**。
#
#   ★ 零命中一定要先分清楚是「現象不存在」還是「測量工具看不到」。
#     這裡兩個原因同時成立：樣本小（366 件、約三個月），而且量尺本身對不上。
#     把「規則沒命中」當成「這種東西不存在」，跟把「沒有客訴」當成
#     「沒有誤擋」是同一種錯 —— 都是拿工具的盲區當結論。
#
#   ⚠️ **不要為此把「当選品」加進 `_CHUSEN`。** 那會讓 Mercari 上大量正常的
#      中籤品轉賣先掉進規則、再靠豁免救回來 —— 先抓再放的路徑越多，
#      出錯機會越多，而今天這個 bug 正是豁免漏接一條造成的。
#      官方商品頁本來就不會寫「当選品」，這條 regex 維持只打官方用字是對的。
