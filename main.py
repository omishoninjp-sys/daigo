...
# ══════════════════════════════════════════════════════════════════════
# 🔴 暫時：圖片抓取探針（驗完移除，見 commit message）
# ══════════════════════════════════════════════════════════════════════
# 用途只有一個：在 Zeabur 機房 IP 上確認 Dior 圖床對「不帶瀏覽器 UA」的回應
# （住宅 IP 是 200 cloudflare，機房 IP 未驗）。detect_invalid_link 擋圖片直連，
# /api/scrape 探不了，所以開這支唯讀端點。
#
# ⚠️ 它會對任意 URL 發伺服器端請求 —— 這是 SSRF 面。驗完立刻移除，不要留著。
# 防護三層（不是只靠 admin key）：
#   1. scheme 只允許 http/https
#   2. 解析 host 的**所有** IP，任一命中私有／loopback／link-local／reserved／
#      multicast／unspecified 就整體拒絕（DNS 可指向內網，所以查解析後的 IP，
#      不是只看主機名）
#   3. 不跟隨轉址（3xx 的 Location 可指向內網）
def _ssrf_guard(url: str):
    """回 (ok, reason, resolved_ips)。任一層不過 ok=False。"""
    import socket
    import ipaddress
    try:
        u = urlparse(url)
    except Exception:
        return False, "URL 無法解析", []
    if u.scheme not in ("http", "https"):
        return False, f"scheme 只允許 http/https（收到 {u.scheme!r}）", []
    host = u.hostname
    if not host:
    ...
    try:
        ...
        return False, f"DNS 解析失敗: {type(e).__name__}: {e}", []
    ips = sorted({info[4][0] for info in infos})
    if not ips:
        return False, "DNS 沒有回任何 IP", []
    for ip_s in ips:
        try:
            ip = ipaddress.ip_address(ip_s)
        except ValueError:
            return False, f"無法解析 IP {ip_s!r}", ips
        # IPv4-mapped IPv6（::ffff:127.0.0.1）要還原成 v4 再判
        if getattr(ip, "ipv4_mapped", None):
            ip = ip.ipv4_mapped
        if (ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved
                or ip.is_multicast or ip.is_unspecified):
            return False, f"解析到內網／保留位址 {ip_s}（拒絕）", ips
    return True, "", ips


@app.get("/api/admin/probe-fetch", dependencies=[Depends(verify_admin_key)])
async def probe_fetch(url: str):
    """🔴 暫時的圖片抓取探針（驗完移除）。對 url 用每個 header profile 各打一次，
    回報 status/content_type/bytes/server —— 不碰 Shopify、不寫任何東西。

    走 _fetch_image（與 _download_b64 同一支 httpx、同一組 header），
    才驗得準；follow_redirects=False（見 _ssrf_guard 第 3 點）。
    """
    from shopify_client import _IMAGE_HEADER_PROFILES, _fetch_image
    ok, reason, ips = _ssrf_guard(url)
    if not ok:
        raise HTTPException(status_code=400, detail=f"SSRF 防護擋下：{reason}")
    results = []
    for name, builder in _IMAGE_HEADER_PROFILES:
        try:
            status, ctype, content, server = await _fetch_image(
                url, builder(url), follow_redirects=False, timeout=10)
            results.append({"profile": name, "status": status,
                            "content_type": ctype, "bytes": len(content or b""),
                            "server": server})
        except Exception as e:
            results.append({"profile": name, "error": f"{type(e).__name__}: {e}"})
    return {"url": url, "resolved_ips": ips, "results": results}
